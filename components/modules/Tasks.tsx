...
export default function Tasks() {
  ...
  const completeTask = async (task: Task) => {
    if (activeTimer === task.id) await stopTimer(task);
    const extraMinutes = parseDurationToMinutes(completionDuration);
    const newTotal = task.time_spent_minutes + Math.max(0, extraMinutes);
    await supabase
      .from("tasks")
      ...
      {showCompleteFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.55)" }}>
          <div className="card-brutal p-5 w-full max-w-md space-y-3">
            ...
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>How much time did it take to complete this task?</p>
